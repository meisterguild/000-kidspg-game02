...
echo.
echo   次にやること
echo     1. 🔴 **インターネットに繋いだまま** 診断と修復.bat を実行し、
echo        ★★★ 問題は見つかりませんでした ★★★ が出るか見る
echo        このバッチと起動バッチが書くのは 4x4 の PNG だけです。
echo        本物のカード土台の読み込み・合成の経路・足りない DLL の複製による
echo        修復は、診断と修復.bat でしか通りません。
echo        ＊ [NG] が出たらネットに繋いだまま繰り返してください。
echo        ＊ 会場でオフラインになってからでは取り返せません。
echo     2. インターネットを切る
echo     3. **PCを再起動する**
echo     4. app\start-kidspg.bat を実行し、★★★ 準備完了 ★★★ が出るか見る
...
echo.
echo   0 件になったら、**ネットに繋いだまま** 診断と修復.bat も通してください
echo   （記念カードの合成経路と DLL の修復は、あちらでしか通りません）。
echo.
