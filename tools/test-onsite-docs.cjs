...
test('閉場の待ち条件は「アプリ内アクティブ」（内部キューは生成中の1枚を数えない）', () => {
  ...
  // 🔴 固定幅で切らない。実測（3巡目）: 節の実長は 769 文字で、1600 の窓は
  //    次の節「持ち帰るもの」まで 831 文字はみ出していた。待ち条件を「内部キュー」に
  //    戻し、正しい文言を次の節へ置くだけで11件すべて緑だった。
  const secEnd = doc.slice(at + 3).search(/\n#{2,3} /);
  assert.ok(secEnd > 0, '「閉場のしかた」の次の見出しが見つかりません');
  const sec = doc.slice(at, at + 3 + secEnd);
