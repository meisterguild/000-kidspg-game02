...
##### 決定的な手がかり: 同じ内容のファイルなのに、片方は動いていた

| | 作成 | SHA256 | 署名 | SAC 下での挙動 |
|---|---|---|---|---|
| 既存 venv の `cython_special.pyd` | 2026-09-01 | `4F7B6A5E…` | NotSigned | **問題なく動いていた** |
| 埋め込み側の同ファイル | 2026-09-09 | `4F7B6A5E…`（**一致**） | NotSigned | 初回ブロック → 再実行で通った |

**中身は完全に同一**です。つまり SAC は「このファイルは危険」と言っているのではなく、
**判定を取りに行っている間だけ止めている**。取り切れば、署名が無いままでも通ります。

##### 対処: SAC はオフにしない。**前日までにオンラインで暖機する**

🔴 **SAC をオフにするのは採らない**（2026-09-09 判断）。この開発機は SAC 有効のまま
ゲームも ComfyUI も動いており、**別PCでも同じ構成にすれば足りる**——ただし
「同じ構成」には**判定を取り切った状態**まで含める必要がある、というのが今回の学び。

| 手順 | 何のため |
|---|---|
| 1. パッケージをコピー（オフラインでよい） | — |
| 2. 🔴 **ネットに繋いで `ウォームアップ.bat`** | アプリと ComfyUI を通し、1枚生成して**未署名ファイルを全部読ませる**。SAC に判定を取らせる |
| 3. ブロックが 0 件になるまで繰り返す | 1回で取り切れないことがある（実測でも2回目で通った） |
| 4. 🔴 **ネットに繋いだまま `診断と修復.bat`** | 2 と 6 が書くのは 4x4 の PNG だけ。**本物のカード土台の読み込み・合成の経路・コーダーの隣へ足りない DLL を複製する修復（`-Fix`）はここでしか通らない**（2026-09-10 の実機ではこの修復で記念カードが作れるようになった）。★★★ 問題は見つかりませんでした ★★★ まで繰り返す |
| 5. ネットを切って**PCを再起動** | 判定がキャッシュに残っているかを確かめる |
| 6. `start-kidspg.bat` で ★★★ 準備完了 ★★★ | **これが当日の形**。ここまで通れば当日も通る |

⚠️ **5〜6 が本当に通るかは別PCで確かめる必要があります。**「判定がオフライン・再起動後も
残る」ことは、この開発機（ずっとオンライン）では確かめられません。
**別PC検証でいちばん重要なのはここです。**
...
| 順 | 退避 |
|---|---|
| 1 | 当日、会場でネットに繋げるなら繋いで `ウォームアップ.bat` |
| 2 | `start-comfyui.bat` は落ちても5秒後に上げ直すので、放置で通ることがある |
| 3 | **AI 変換を切る**（`config.json` の `comfyui` セクションを外す）。カードの絵は全員同じプレースホルダになるが、**ゲームとカードとランキングは動く**（README の「AI変換を当日オフにする」） |

##### 当日その場で原因が見えるようにした

SAC に止められたときの見え方は、ComfyUI のログに
`ImportError: DLL load failed ...` の1行が残るだけでした。これでは当日スタッフが
原因に辿り着けません。そこで `start-kidspg.bat` が ComfyUI の起動失敗と準備確認の
時間切れのときに **イベントログを読み、「SAC が N 件をブロックしています」と
ファイル名つきで出す**ようにしました（`tools/onsite/check-sac-blocks.ps1`）。
