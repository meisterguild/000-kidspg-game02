...
- `comfyui.retry` で**作り直す**（2026-09-09 に実装）。対象はアップロードの失敗・投入の失敗・
  通信の失敗・出力の取りこぼし。**キュー期限（`timeouts.queue`）の超過では作り直さない**——
  期限切れは ComfyUI が詰まっている状態で、積み直すと待っている全員がさらに遅れるため。
  既定は3回・1秒間隔。作り直しているあいだはランキング側へ「作り直します（n回目）」が出る。
